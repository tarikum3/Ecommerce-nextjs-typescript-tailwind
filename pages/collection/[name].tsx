...
import { ProductCard } from '@components/product'
  ...
  export async function getStaticProps({
   ...
   const currentCategory = categories?.find((cat: any) => cat.slug == params!.name)
    let products
  ...
  export async function getStaticPaths({ locales }: GetStaticPathsContext) {
    ...
    return {
      paths:
      categories?.map((category: any) => `/collection/${category.slug}`),
      fallback: 'blocking',
    }
  }

  export default function Collection({
    products,found,collection
    ...
    return (
      <div className=" m-10 ">
      <p className='text-5xl text-secondary text-center font-bold m-5 m-24'> {collection} </p>
